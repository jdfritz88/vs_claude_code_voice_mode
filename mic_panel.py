...
class MicControlPanel:
    ...
    def _start_recording(self):
        ...
        volume_multiplier = self.volume.get() / 50.0
        ...
        def recording_callback(indata, frames, time_info, status):
            ...
            if current_mode != "push_to_talk" and self.is_muted:
                return
            scaled = (indata.copy().astype(np.float32) * volume_multiplier).astype(np.int16)
            with self._recording_lock:
        ...
        try:
            ...
            self._recording_stream = sd.InputStream(**kwargs)
            self._recording_stream.start()
            logger.info("Recording started (InputStream opened)")
        except Exception as e:
            logger.error(f"Failed to start recording: {e}")
            self.is_recording = False
            self.status_label.config(text=f"Mic error: {e}", fg="#ff4444")
    ...
    def _stop_recording(self):
        ...
        if self._recording_stream is not None:
            try:
                ...
                logger.warning(f"Error stopping recording stream: {e}")
            self._recording_stream = None
        ...
        with self._recording_lock:
            has_frames = len(self._recording_frames) > 0
        ...
        if has_frames:
            ...
            thread.start()
            logger.info("Recording stopped, processing thread started")
        else:
    ...
    def _process_recording(self):
        ...
        try:
            ...
            logger.info(f"Processing {duration:.1f}s of recorded audio")
